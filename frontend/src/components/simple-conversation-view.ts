...
export class SimpleConversationView extends LitElement {
	...
	private formatStringContent(content: string): TemplateResult {
		...
		// Extract and remove escaped system-reminder blocks
		const escapedRegex = /&lt;system-reminder&gt;([\s\S]*?)&lt;\/system-reminder&gt;/g;
		let match;
		while ((match = escapedRegex.exec(content)) !== null) {
			systemReminders.push(match[1].trim());
		}
		// Create fresh regex for replacement
		mainContent = mainContent.replace(/&lt;system-reminder&gt;[\s\S]*?&lt;\/system-reminder&gt;/g, "");

		// Extract and remove raw system-reminder blocks
		const rawRegex = /<system-reminder>([\s\S]*?)<\/system-reminder>/g;
		while ((match = rawRegex.exec(content)) !== null) {
			systemReminders.push(match[1].trim());
		}
		// Create fresh regex for replacement
		mainContent = mainContent.replace(/<system-reminder>[\s\S]*?<\/system-reminder>/g, "").trim();
