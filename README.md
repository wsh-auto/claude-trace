...
Logs are saved to `~/.claude/trace/{project-path}/log-YYYY-MM-DD-HH-MM-SS.{jsonl,html}` where `{project-path}` mirrors your working directory structure (e.g., `/Users/you/project` becomes `-Users-you-project`). The HTML file is self-contained and opens in any browser without needing a server.
...
- Scans all `.jsonl` log files in the trace directory
...
1. **Backend** (`src/`)
   ...
   - **Interceptor** (`interceptor.ts`) - injects itself into Claude Code, intercepts calls to fetch(), and logs them to JSONL files in `~/.claude/trace/{project-path}/`.
